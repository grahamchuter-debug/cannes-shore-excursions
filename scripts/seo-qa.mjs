#!/usr/bin/env node
/**
 * Post-implementation SEO QA for Cannes Shore Excursions static export.
 */
...
const SITE_URL = "https://cannesshoreexcursions.com";
...
const important = ["/", "/shore-excursions/", "/cruise-port-guide/", "/cruise-planner/", "/faq/", "/shore-excursions/monaco-magic/"];
...
if (canonicalIssues === 0) pass("Canonical tags point to https://cannesshoreexcursions.com (non-www)");

const redirects = readText(join(ROOT, "public/_redirects"));
if (redirects.includes("www.cannesshoreexcursions.com") && redirects.includes("301")) {
  pass("WWW → non-WWW redirects configured in public/_redirects");
} else {
  fail("WWW → non-WWW redirects missing from public/_redirects");
}
if (redirects.includes("http://cannesshoreexcursions.com")) {
